...
Namespace Personify.ApplicationManager
    Public Class ExportExcel
        ' Methods
        Private Sub CreateStylesheet(ByVal writer As XmlTextWriter, ByVal sHeaders As String(), ByVal sFileds As String(), ByVal FormatType As PersonifyEnumerations.ExportType)
            Try
                ...
                Do While (i <= num3)
                    ...
                    If (i <> (sFileds.Length - 1)) Then
                        writer.WriteString(Conversions.ToString(Interaction.IIf((FormatType = PersonifyEnumerations.ExportType.CSV), ",", ChrW(9))))
                    End If
                    i += 1
                Loop
        ...
        Public Sub ExportData(ByVal datasource As DataTable, ByVal FormatType As PersonifyEnumerations.ExportType, ByVal FileName As String)
            Try
        ...
        Public Sub ExportData(ByVal datasource As DataTable, ByVal ColumnList As Integer(), ByVal FormatType As PersonifyEnumerations.ExportType, ByVal FileName As String)
            Try
                If (datasource.Rows.Count = 0) Then
                    Throw New Exception("There is no data to export")
                End If
        ...
        Public Sub ExportData(ByVal datasource As DataTable, ByVal ColumnList As Integer(), ByVal Headers As String(), ByVal FormatType As PersonifyEnumerations.ExportType, ByVal FileName As String)
            Try
                If (datasource.Rows.Count = 0) Then
                    Throw New Exception("There is no data to export")
                End If
        ...
        Private Sub ExportXSLT(ByVal dsExport As DataSet, ByVal sHeaders As String(), ByVal sFileds As String(), ByVal FormatType As PersonifyEnumerations.ExportType, ByVal FileName As String)
            Try
                ...
                If (FormatType = PersonifyEnumerations.ExportType.CSV) Then
                    Me.response.ContentType = "text/csv"
                    Me.response.AppendHeader("content-disposition", ("attachment; filename=" & FileName))
                ElseIf (FormatType = PersonifyEnumerations.ExportType.Excel) Then
