...
Namespace Controls.Common
    ''' <summary>
    ''' Prepresent GUI for OrganizationInvolvement tab 
    ''' </summary>
    Public Class OrganizationInvolvement
...
#Region "Event Handlers"
        ...
        Protected Sub Page_Load(ByVal sender As Object, ByVal e As EventArgs) Handles Me.Load
            Try
                AddEventHandlers()
                ProgramType = ActivityProgramType.ORGINVOLVE.ToString()
        ...
        Public Overrides Function Save() As IIssuesCollection
            ...
            Try
                ...
                If Not String.IsNullOrEmpty(ConflictMessage) Then '' validation rule is conflicted
                    ...
                    Return iissueResult
                End If
                If RuleID = ValidationRuleId.RECERT_ORGAN_INVOL_MIM_MAX_ABNN.ToString() Then
                ...
                End If
                If rptInvolvement.Items Is Nothing OrElse rptInvolvement.Items.Count <= 0 Then
                    '' if check rule and set ceMin = 0 => pass basic rule
                    If RuleID = ValidationRuleId.RECERT_ORGAN_INVOL_MIM_MAX_ABNN.ToString() AndAlso totalCEMin = 0 Then
                        hdIsIncomplete.Value = CommonConstants.TAB_COMPLETED
                    Else
                        hdIsIncomplete.Value = CommonConstants.TAB_INCOMPLETED
                    End If
        ...
        Protected Sub rptInvolvement_ItemCommand(ByVal source As Object, ByVal e As System.Web.UI.WebControls.RepeaterCommandEventArgs) Handles rptInvolvement.ItemCommand
            Try
        ...
        Protected Sub rptInvolvement_ItemDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.RepeaterItemEventArgs) Handles rptInvolvement.ItemDataBound
            Try
                If e.Item.ItemType = ListItemType.AlternatingItem OrElse e.Item.ItemType = ListItemType.Item Then
                    ...
                    If userdine IsNot Nothing Then
                        '' bind CE Type
                        Dim point As String
                        Dim lblCEType As Label = CType(e.Item.FindControl("lblTypeOfCE"), Label)
        ...
        Private Sub BindingDataToList()
            Try
                ...
                If activityExternals IsNot Nothing Then
                    ...
                    End If
                End If
                Me.rptInvolvement.DataSource = activityExternals
                Me.rptInvolvement.DataBind()
        ...
        Private Sub SetPropertiesForObject(ByRef objectItem As IUserDefinedCustomerCEActivity)
            If objectItem IsNot Nothing Then
                ...
                With objectItem
                    ...
                    .ProgramTitle = Me.txtTitle.Text
                    .OrganizationProviding = _txtFullName.Text
                    .ProgramDate = DateTime.MinValue
                    If Me.txtNumberOfHours.Text <> String.Empty AndAlso CommonHelper.CheckIsNumber(Me.txtNumberOfHours.Text) Then
                        .CEHours = Decimal.Parse(Me.txtNumberOfHours.Text)
                    Else
...
#Region "validation"
        Public Function CheckConflictValidation(ByRef ruleID As String,
            ...
            Dim validationABNN_MinMaxItem = ModuleConfigurationHelper.Instance.IsValidationRuleEnabled(
                                                                ValidationRuleId.RECERT_ORGAN_INVOL_MIM_MAX_ABNN.ToString(),
                                                                Server.MapPath(ParentModulePath))
            conflictMessage = String.Empty
            ruleID = String.Empty
            If validationABNN_MinMaxItem = True Then
                ruleID = ValidationRuleId.RECERT_ORGAN_INVOL_MIM_MAX_ABNN.ToString()
                Return False
            End If
