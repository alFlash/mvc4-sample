...
#Region "Event Handlers"
        ...
        Protected Sub Page_Load(ByVal sender As Object, ByVal e As EventArgs) Handles Me.Load
            Try
                AddEventHandlers()
                ProgramType = ActivityProgramType.RESEARCH.ToString()
        ...
        Public Overrides Function Save() As IIssuesCollection
            ...
            Try
                hdIsIncomplete.Value = CommonConstants.TAB_INCOMPLETED
                Dim results = amcCertRecertController.CommitCustomerExternalCEActivity(ProgramType,
                ...
                If Not String.IsNullOrEmpty(ConflictMessage) Then '' validation rule is conflicted
                    ...
                    Return iissueResult
                End If
                If RuleID = ValidationRuleId.RECERT_RESEARCH_MIM_MAX_ABNN.ToString() Then
                ...
                End If
                If rptResearch.Items Is Nothing OrElse rptResearch.Items.Count <= 0 Then
                    '' if check rule and set ceMin = 0 => pass basic rule
                    If RuleID = ValidationRuleId.RECERT_RESEARCH_MIM_MAX_ABNN.ToString() AndAlso totalCEMin = 0 Then
                        hdIsIncomplete.Value = CommonConstants.TAB_COMPLETED
                    Else
                        hdIsIncomplete.Value = CommonConstants.TAB_INCOMPLETED
                    End If
        ...
        Protected Sub rptResearch_ItemCommand(ByVal source As Object,
                                              ByVal e As System.Web.UI.WebControls.RepeaterCommandEventArgs
                                              ) Handles rptResearch.ItemCommand
            Try
                If e.CommandName.Equals("Delete") Then
                    hdIsIncomplete.Value = CommonConstants.TAB_INCOMPLETED
                    Dim customerCEActivityItem As IUserDefinedCustomerCEActivity
                    customerCEActivityItem = amcCertRecertController.GetCustomerExternalCEActivityByGuiId(e.CommandArgument.ToString(),
                                                                                                          ProgramType,
        ...
        Protected Sub rptResearch_ItemDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.RepeaterItemEventArgs) Handles rptResearch.ItemDataBound
            Try
...
#Region "Private Methods"
        ...
        Private Sub BindingDataToList()
            Try
                ...
                activityExternals = AMCCertRecertController.GetExternalActivityList(ProgramType,
                                                                                    Me.MasterCustomerId,
                                                                                    Me.SubCustomerId)
                If activityExternals.Count > 0 Then
                    ...
                    End If
                End If
                Me.rptResearch.DataSource = activityExternals
                Me.rptResearch.DataBind()
        ...
        Private Sub SaveCEActivity()
            ...
            Try
                ...
                If (Me.hdCurrentObjectUniqueId.Value <> String.Empty) Then
                    customerExternalCEActivityItem =
                                        AMCCertRecertController.GetCustomerExternalCEActivityByGuiId(
        ...
        Private Sub SetPropertiesForObject(ByRef objectItem As IUserDefinedCustomerCEActivity)
            If objectItem IsNot Nothing Then
                ...
                With objectItem
                    ...
                    .ProgramTitle = txtResearch.Text
                    .ProgramDate = DateTime.MinValue
                    If CType(Me.txtCompletionDate, AMCDatetime).Text <> String.Empty Then
                        .ProgramDate = DateTime.Parse(CType(Me.txtCompletionDate, AMCDatetime).Text)
                    End If
                    .OrganizationProviding = txtOrganization.Text
...
#Region "validation"
        Public Function CheckConflictValidation(ByRef ruleID As String,
            ...
            Dim validationABNN_MinMaxItem = ModuleConfigurationHelper.Instance.IsValidationRuleEnabled(
                                                                ValidationRuleId.RECERT_RESEARCH_MIM_MAX_ABNN.ToString(),
                                                                Server.MapPath(ParentModulePath))
            conflictMessage = String.Empty
            ruleID = String.Empty
            If validationABNN_MinMaxItem = True Then
                ruleID = ValidationRuleId.RECERT_RESEARCH_MIM_MAX_ABNN.ToString()
                Return False
            End If
